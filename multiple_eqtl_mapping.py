...
def get_snp(snp, expr_ids, vcf_file):
    ...
    for snp_line in tabix_fetch:
        if snp_line.id == snp:
            if 'DS' in snp_line.format:
                ...
                snp_line = [snp_line.samples[z]['GT'] for z in expr_ids]
...
def run_analysis(args):
    ...
    if args.genes == 0:
        ...
        forward_file = "%s.%s" % (args.forward, args.jobnumber)
        backward_file = "%s.%s" % (args.backward, args.jobnumber)
    ...
    if args.cov:
        with open(args.cov) as f:
            ...
            if sum(1 for x in expr_ids if x not in cov_ids[1: ]) != 0:
                ...
                sys.exit(1)
            places = [0]
            places.extend([cov_ids.index(x) for x in expr_ids])
        ...
        with open(forward_file, 'w') as results_file:
            for line in results_iter:
                ...
                genes[line[0]] = {line[snp_position]:
                                  (get_snp(line[snp_position], expr_ids, vcf_file), 1)}
                discovered = True
                iteration = 1
                        ...
                        with open(temp) as tissue_file:
                            for tissue_line in (line.strip().split()
                                ...
                                if tissue_line[0] == line[0] and float(tissue_line[beta_position]) <= threshold:
                                    snp = get_snp(
                                        tissue_line[snp_position], expr_ids, vcf_file)
